...
for (const text of [html, css, renderer, main]) assert.equal(/devuser|local-ai|be-a-fool/i.test(text), false, 'UI contains a personal path');
for (const required of [
  ...
  'Review selected files', 'Download selected', 'download-progress', 'contextIsolation:true', 'sandbox:true'
