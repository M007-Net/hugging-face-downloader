...
const state = { page:'new', status:null, catalog:null, link:'', loading:false, mode:'quant', bit:0, quant:'', model:'', vision:'', mtp:'', manual:new Set(), search:'', remember:true, job:null };
const $ = id => document.getElementById(id);
const esc = value => String(value ?? '').replace(/[&<>"']/g,c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const size = value => { if (!value) return '0 B'; const n = Math.min(4, Math.floor(Math.log(value)/Math.log(1024))); return `${(value/1024**n).toFixed(n > 1 ? 2 : 0)} ${['B','KB','MB','GB','TB'][n]}`; };
...
function notify(message, good=false) { $('notice').textContent=message; $('notice').className=good?'success':'error'; $('notice').hidden=!message; }
...
function resetQuant() {
  ...
  state.mode=state.catalog.info.isFile || !list.length?'manual':'quant'; state.manual = new Set(state.catalog.info.isFile ? [state.catalog.info.subpath]:[]);
...
function render() {
  ...
  $('version').textContent='v'+state.status.version; $('engine-pill').textContent=state.status.aria?'● Download engine ready':'Set up download engine'; $('engine-pill').classList.toggle('ready',!!state.status.aria);
  $('queue-badge').textContent=active()?'1':'';
  $('app').innerHTML=state.page==='new'?newPage():state.page==='downloads'?downloadsPage():settingsPage();
}
function newPage() {
  const c=state.catalog; const selected=chosen(); const total=selected.reduce((n,f)=>n+f.size,0);
  return `<div class="heading"><div class="eyebrow">BUILD YOUR LOCAL LIBRARY</div><h1>Your next model, made simple.</h1><p>Choose a model, pick the files you need, and save them where you want.</p></div><div class="layout"><div class="flow">
  <section class="card" data-help="repo"><div class="section-title"><h2><span class="step">1</span> Find your model</h2><span class="caption">Hugging Face</span></div><form id="repo-form"><label for="repo">Model link or repository name</label><div class="row field-note"><input id="repo" placeholder="huggingface.co/owner/model-GGUF" value="${esc(state.link)}" ${active()?'disabled':''}><button class="primary" type="submit" ${state.loading||active()?'disabled':''}>${state.loading?'Loading…':'Load model'}</button></div></form>${c?`<div class="repo-name">✓ ${esc(c.info.repo)} <span class="caption"> · ${esc(c.info.rev)} · ${c.files.length} files</span></div>`:'<div class="hint">Repository links, folder links, and direct file links all work.</div>'}</section>
  ${active()?'<div class="busy-note">A download is running. Visit Downloads to follow its progress.</div>':''}
  ${c?`<section class="card" data-help="quant"><div class="section-title"><h2><span class="step">2</span> Choose what to download</h2></div><div class="segmented" role="group" aria-label="Selection mode"><button data-mode="quant" class="${state.mode==='quant'?'selected':''}" ${!c.bundles.some(b=>b.quant)?'disabled':''}>Choose quantization</button><button data-mode="manual" class="${state.mode==='manual'?'selected':''}">Choose specific files</button></div>${state.mode==='quant'?quantFields():manualFields()}</section>
  ...
  '<section class="empty"><div class="empty-symbol">↓</div><h2>A clear path from model to download.</h2><p>Load a repository to see available quantizations, file sizes, and optional companions in one place.</p><div class="empty-steps"><span>Choose a quant</span><span>Add companions</span><span>Download</span></div></section>'}
...
function companion(key,title,explanation) {
  ...
  return `<div class="companion"><label class="check"><input type="checkbox" data-companion="${key}" ${state[key]?'checked':''} ${!list.length?'disabled':''}>${title}<span class="tag ${list.length?'':'neutral'}">${list.length?'DETECTED':'NOT FOUND'}</span></label><small>${list.length?explanation:'No separate file detected here. It may be embedded or hosted elsewhere.'}</small>${state[key]?`<select aria-label="${title} file" data-extra="${key}">${list.map(b=>option(b.name,`${b.name} · ${size(b.size)}${b.complete?'':' · missing parts'}`,state[key])).join('')}</select>`:''}</div>`;
}
...
document.addEventListener('submit',e=>{e.preventDefault();act(async()=>{
  ...
  if(e.target.id==='settings-form'){const v={outputDir:$('setting-output').value,aria2Path:$('setting-aria').value,connections:Number($('setting-connections').value),disableIPv6:$('setting-ipv4').checked,quant:$('setting-quant').value.trim()};if($('setting-token').value)v.token=$('setting-token').value;state.status=await api.saveSettings(v);render();notify('Settings saved on this computer.',true);}
});});
document.addEventListener('click',e=>{const t=e.target.closest('button');if(!t)return;act(async()=>{
  ...
  if(t.id==='select-visible'){state.catalog.files.filter(f=>f.path.toLowerCase().includes(state.search.toLowerCase())).forEach(f=>state.manual.add(f.path));render();}
...
act(async()=>{state.status=await api.status();state.job=state.status.job;render();});
